...
internal class LoaderGUIStuff
{
	...
	private static Bitmap[] menuIcons;
	...
	{
		var WorldImage = MainPanel.GetControlFromPosition(0, 0);
		Color borderC;
	...
	public static void BasicButtonSetup(Button Button, int FontSize, int Width, int Height)
	{
		...
		Button.Click += (sender, e) =>
		{
			...
			var clickDelayTimer = new Timer(10);
		...
		Buttons[1].Click += (sender, e) =>
		{
			...
			try
			{
				...
				HelperStuff.LoadImage(@"WorldScreenshots\World" + int.Parse(WorldImage.Text)).Save(
					...
					(Worlds.Length - 1) + ".png", ImageFormat.Png);
			}
			catch (Exception)
			{
			}
	...
	async public static void SetupSettingsPanel()
	{
		...
		var MainPanel = SetupBasicRightPanel(5, 5);
	...
	async public static void SetupMapEditorPanel()
	{
		...
		var MainPanel = SetupBasicRightPanel(5, 4);
		...
		buttons[6].Click += (sender, e) =>
		{
			...
			for (var i = 0; i < newWorld.Map.Length; i++)
			{
	...
	async private static void SetupLeft(MyTableLayoutPanel MainPanel)
	{
		...
		foreach (var B in LeftMenuButtons)
		{
			BasicButtonSetup(B, LoaderFontSize, 302, 100);
		}

		var LeftPanel = new MyTableLayoutPanel();
		...
		for (var i = 2; i < LeftPanel.RowCount; i++)
		{
	...
	async private static void SetupDeletePanel(MyTableLayoutPanel ParentPanel)
	{
		...
		Buttons[1].Click += (sender, e) =>
		{
			...
			for (var i = int.Parse(WorldImage.Text); i < Worlds.Length; i++)
			{
				// save the image
				var curImg = HelperStuff.LoadImage(@"WorldScreenshots\World" + (i + 1));
				curImg.Save(
	...
	{
		var WorldImage = MainPanel.GetControlFromPosition(0, 0) as Label;
