    ...
    class HelperStuff
    {
        ...
        public static async Task UpdateFontNew(Control control)
        {
            ...
            using (Graphics g = control.CreateGraphics())
            {
                ...
                Font testFont = new Font(control.Font.FontFamily, fontSize);

                // Measure the string size in the current font
                SizeF stringSize = g.MeasureString(control.Text, testFont);

                // Calculate the font scaling factor
                float widthRatio = (float)Math.Floor(control.Width * 0.75) / stringSize.Width;
                float heightRatio = (float)Math.Floor(control.Height * 0.75) / stringSize.Height;
		...
		public static void SetupMouseEffects(Control control, bool MouseHoverEffects, bool MouseClickEffects, bool ButtonBorderEffects)
		{
			...
			if (ButtonBorderEffects && control is Button button)
			{
				...
				control.MouseEnter += (s, e) =>
				{
					...
					if (button != null && ((int[])button.Tag)[2] == 0 && ((int[])button.Tag)[3] == 0 && ((int[])button.Tag)[4] == 0)
					{
                        ((int[])button.Tag)[4] = 1;
                        ((int[])button.Tag)[2] = 1;
						...
						{
							if (button.FlatAppearance.BorderSize < ((int[])button.Tag)[1] && ((int[])button.Tag)[3] == 0)
							{
							...
							else
							{
								...
								Point mousePosition = button.PointToClient(Cursor.Position);
								...
								{ 
                                    MethodInfo onMouseLeaveMethod =
                                    typeof(Control).GetMethod("OnMouseLeave", BindingFlags.NonPublic | BindingFlags.Instance);
				...
				{
					Button button = s as Button;
					if (button != null && ((int[])button.Tag)[2] == 1 && ((int[])button.Tag)[3] == 0 && ((int[])button.Tag)[4] == 0)
					{
                        ((int[])button.Tag)[4] = 1;
                        ((int[])button.Tag)[2] = 0;
                        Timer timer = new Timer();
						...
						timer.Tick += (sender, args) =>
						{
							if (button.FlatAppearance.BorderSize > ((int[])button.Tag)[0] && ((int[])button.Tag)[3] == 0)
							{
							...
							else
							{
                                ...
                                if (button.ClientRectangle.Contains(mousePosition))
                                {
                                    ...
                                    MethodInfo onMouseEnterMethod =
                                    typeof(Control).GetMethod("OnMouseEnter", BindingFlags.NonPublic | BindingFlags.Instance);
        ...
        public static Task AnimatePanelBounds(Control panel, Rectangle newBounds, int duration)
        {
            ...
            Timer timer = new Timer { Interval = duration / 30 };
            timer.Disposed += (sender, e) => tcs.SetResult(null);

            double stepCount = (double)duration / timer.Interval;
            double stepX = (newBounds.X - panel.Left) / stepCount;
            double stepY = (newBounds.Y - panel.Top) / stepCount;
            double stepWidth = (newBounds.Width - panel.Width) / stepCount;
            double stepHeight = (newBounds.Height - panel.Height) / stepCount;
