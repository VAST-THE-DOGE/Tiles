...
namespace Tiles
{
	internal class GithubHelper
	{
		...
		public static void ShowError(Exception error, string errorLevel, string gameState)
		{
			...
			var box = MessageBox.Show($"An error occurred, do you wish to submit a generated bug report to help fix it?\nError:\n {error.Message}", "Report Error/Bug?", MessageBoxButtons.YesNo, MessageBoxIcon.Error);
			if (box == DialogResult.Yes)
			{
				try
				{
					...
					{
						FileName = $"{GithubProject}/issues/new?title={title}&body={body}&labels=bug&assignees=vast-the-doge",
		...
		public static async Task CheckForUpdate()
		{
			...
			{
				MessageBox.Show($"You are using a pre release version! Expect bugs and unfinished features.\n    Latest Version: {latestGitHubVersion}\n    Current Version: {GlobalVariableManager.VERSION}", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
